...
tok='6618915532:AAGgSYzx_fNfdmsOqpIXuqVXl7VfXZUFXvQ'
...
groups=['O7650','sis_fff','friendawe','SUSU8U']
...
Copyright=['@VnVn6','@q_B_h','Q_b_h','TeaM_ExEx','@VnVn2','WARRIOR','Gxp6','YsYsD','@MPMPK','Qredes','tf_r7']
...
if foo:
	...
	cobe="#Decoded By Hso : @sis_f  On Bot : @sis_ff_bot \n\n"
	source=cobe+source+'\n'+cobe
	#source=source.split('        ')
	#source=''.join(source)
...
#	       source=source.replace('# File: AFRIT(0.1)(2).py (Python 3.9)','#decoded by @sis_f')
	       ...
	       if len(source) > 250:
	       	...
	       	if 'std::bad_cast' in file:
	       			...
	       			if fs > 0:
	       				...
	       				bot.reply_to(reb,f'file decoded By @sis_f\n\nLayers `{la}`')
	       				la=0
...
def als(om,m):
	...
	if om:
		try:
			error_msg='لا يعمل البوت فقط في هاذه القروب @sis_fff \nارسل ملفك في القروب ليتم فك تشفيره'
